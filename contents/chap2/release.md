...
3. **Develop the package** by writing the source code and tests, and building the documentation.
4. **Open-source the package** by pushing the package to a public repository on GitHub.
5. **Register the package** in the `General` registry by creating a pull request to the `General` registry. This process can be automated by the [Julia registrator](https://github.com/JuliaRegistries/Registrator.jl).
6. Wait for the pull request to be merged. After that, your package is available to the public. A good practice is to **tag a release** after the pull request is merged so that your package version update can be reflected in your GitHub repository. This process can be automated by the [TagBot](https://github.com/JuliaRegistries/TagBot).
...
### Case study: The structure of [OMEinsum.jl](https://github.com/under-Peter/OMEinsum.jl)
